Lab 2 README
==========================================================

LEADER
akuru003@ucr.edu 

MEMBERS
==========================================================
akuru003@ucr.edu 
jlee368@ucr.edu

REMARKS
- My module works as described in the specification. I ran all the test cases and had no errors.
==========================================================

BUGS IF ANY 
- There are no known bugs as the test cases all came out fine. 

==========================================================

ORIGINAL WORK STATEMENT
- I certify that this submission represents my own original work. This submission was not done in collaboration nor was any substanial portion of the code obtained from third parties, including websites and the like.
