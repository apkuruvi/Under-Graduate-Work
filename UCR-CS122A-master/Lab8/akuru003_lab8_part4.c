 ...
 *	Exercise Description: Motor controlled by keypad as well as direction
 *	
...
void LEDS_Tick(){
	switch(led_state)
	{
		...
		if (GetBit(PINC,3)==0) { arr[j] = '0'; PORTD = 0x00; j++; }
		...
		if (GetBit(PINC,3)==0) { arr[j] = '#'; PORTD = 0x00; j++;}
		
		PORTC = 0x7F;
		asm("nop");
		if (GetBit(PINC,0)==0) { arr[j] = ' '; j = 0; PORTD = 0xFF; led_state = s1; pos = 0;}
		if (GetBit(PINC,1)==0) {  arr[j] = ' '; j = 0; PORTD = 0xFF; led_state = s1; pos = 1;}
