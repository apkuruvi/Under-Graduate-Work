...
int ar[8][8]; // 1-blue, 2-red, 3-green 4 violet 5 yellow 6 light blue
...
enum states {start, donothing, xread, yread} state;
unsigned short xval,yval,joystickread;
void tick()
{
	switch(state)
	{
...
int main(void)
{
	...
	{	
		...
		counter5++;
		counter6++;
		if(joystickread == 2 || joystickread == 1 || joystickread == 8 || joystickread == 4)
		{
			seed += 1;
			srand(seed);
		}
